...
class TestFeedbackModule:
    ...
    async def test_feedback_creation_success(self, client, authenticated_user):
        ...
        async with client as c:
            response = await c.post("/feedback", json=feedback_data, headers=authenticated_user)
    ...
    @pytest.mark.asyncio
    async def test_admin_can_view_all_feedback(self, client, authenticated_admin, authenticated_user):
        """Test that admin can view all feedback entries"""
        # Create some feedback as regular user
        feedback_data = {
            "rating": 4,
            "comment": "Admin should see this"
        }
    ...
    async def test_multiple_feedback_from_same_user(self, client, authenticated_user):
        ...
        async with client as c:
            for feedback in feedback_list:
                response = await c.post("/feedback", json=feedback, headers=authenticated_user)
                assert response.status_code == 201
                
            # Check that all feedback was created
            summary_response = await c.get("/feedback/summary", headers=authenticated_user)
